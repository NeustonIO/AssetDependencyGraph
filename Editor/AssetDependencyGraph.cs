...
namespace Neuston
{
	...
	{
		private readonly Dictionary<string,List<string>> targetToSourceDictionary;
		...
		private static Dictionary<string, List<string>> CreateTargetToSourceDictionary()
		{
		...
		public Dictionary<string, List<string>> GetDependants(IEnumerable<string> selectedObjects)
		{
			...
			{
				if (targetToSourceDictionary.ContainsKey(selectedObject))
				{
					results.Add(selectedObject, targetToSourceDictionary[selectedObject]);
				}
				else
				{
